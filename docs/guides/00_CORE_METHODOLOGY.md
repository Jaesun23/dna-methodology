...
| Stage | 역할 정립 | 가이드 문서 | 상태 |
|:-----:|:--------:|:-----------:|:----:|
| 1-6 | ✅ 완료 | ✅ v4.1 완료 | Stage 가이드 완성 |
| 7-9 | ✅ 완료 | ✅ v4.1 완료 | Stage 가이드 완성 |

```
✅ Gemini 연구(8개 문서)를 통해 Stage 1-9의 역할이 명확해짐
✅ 전체 9-Stage 가이드 문서 작성 완료 (v4.1)
✅ DNA 4대 핵심 원칙 확정 (2025-12-03)
⏳ 2호 기능(Skills, Commands, Agents)과 조합한 프레임워크화 진행 중
```
...
## Part 2: DNA 4대 핵심 원칙 ⭐ (2025-12-03 확정)
...
────────────────────────────────────────────────────────
Stage 1-7: Human-Driven (문서 크기 제한 없음)
Stage 8-9: AI-Driven (80-90K 토큰 내에서 완료 가능해야!)

체크리스트: 100-150줄 (Task 유형별 다름)
```
...
검증 1: 크기
├─ 체크리스트 80-180줄? 예상 시간 2-4시간?
└─ 실패 시 → 분할 또는 합치기

검증 2: 의존성
├─ 선행 Task 명확? 순환 의존성 없음?
└─ 실패 시 → 의존성 재설계
...
❌ 레이어별 (Anti-pattern):
Task 001: 모든 엔티티 → Task 002: 모든 리포지토리 → ...
문제: Task 001 완료해도 "작동하는 기능" 없음!

✅ 기능별 (Best Practice):
Task 001: User 기능 (E2E) → Task 002: Order 생성 (E2E) → ...
장점: Task 001 완료 = User 기능 작동!

연결부 (Interface) 먼저 정의:
────────────────────────────────────────────────────────
Task 000: 인터페이스 정의 (IUserRepository, IOrderService, ...)
이후 Task: 인터페이스 구현 + E2E 작동!
```
...
├─ 오류 미발견 시 → Stage 8-9 전체 재작업 (10시간+)
├─ 오류 수정 시 → Blueprint만 수정 (2-3시간)
└─ 비용 차이: 10배+

추적성: docs/revision_log.md에 모든 수정 이력 기록
```

---

## Part 3: DNA 방법론의 2가지 실행 원칙
...
| 질문 | 의미 | 예시 |
|:----:|:----:|:----:|
| **Q1** | 컨텍스트 한계 고려 | Task 100줄씩, ADR 3개씩 |
...
품질 기준: Lint 0, Type 0, Coverage 95%+
```

---

## Part 6: 3-Tier 문서 구조 (Guide / Manual / Cases)

각 Stage마다 **3종류의 문서**가 역할을 나누어 제공됩니다:
...
> **"간결하되 이해 가능해야 한다. 단어 나열이 아닌 맥락 있는 설명."** - Jason

**문제**: 모든 내용을 한 문서에 담으면 컨텍스트 초과
**해결**: 역할별로 분리하여 필요한 깊이만 참조

### 각 Tier의 역할

| Tier | 역할 | 목적 | 분량 | 톤 |
|------|------|------|------|-----|
| **Guide** | 실행용 템플릿 | 직접 작성 가능하도록 | 1000-1500줄 | 실행 지향적, 간결 |
| **Manual** | 심화 학습용 | "왜?"와 "어떻게?" 상세 설명 | 제한 없음 | 교육적, 상세 |
| **Cases** | 실전 학습용 | 전체 흐름을 실제 프로젝트로 | 제한 없음 | 실전적, 구체적 |
...
분량: 1000-1500줄 (AI 컨텍스트 윈도우 고려)
내용: 질문 + 작성 가이드 + 간단한 예시
참조: Manual/Cases로 링크 ("더 알고 싶다면...")
```

**2. Manual (해설서)**
```
목적: "왜 이렇게 하는가?"에 대한 상세 설명
분량: 필요한 만큼 (제한 없음)
...
├─ Guide 읽기 → 바로 작업 시작
├─ 막히면 → Manual 참조 (왜/어떻게)
└─ 전체 감 잡고 싶으면 → Cases 참조 (실전 예시)

AI 협업 시:
├─ AI에게 Guide만 제공 (컨텍스트 효율)
├─ 특정 부분 설명 필요 시 → Manual 해당 섹션
└─ 패턴 학습 필요 시 → Cases 해당 부분
```

### 현재 문서 현황

| Stage | Guide | Manual | Cases |
|-------|-------|--------|-------|
| 1 | ✅ 01G-00 | ⏳ | ⏳ |
| 2 | ✅ 02G-00 | ⏳ | ⏳ |
| 3 | ✅ 03G-00 | ⏳ | ⏳ |
...
### 🔑 DNA 4대 핵심 원칙 (2025-12-03 확정)
```
1. 🎯 AI 최적 크기: 80-90K 토큰, 체크리스트 100-150줄
2. 🔄 완전해질 때까지 반복: 3단계 검증 (크기, 의존성, 완전성)
3. 🧩 기능별 분해 + 연결부 + 조립: 레이어별 X, 기능별 E2E O
4. ⏪ 역방향 수정 프로토콜: 6단계, Stage 7이 통합의 정점
```
